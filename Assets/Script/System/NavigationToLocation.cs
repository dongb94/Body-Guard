...
public class NavigationToLocation : MonoBehaviour
{
	...
	void Awake ()
	{
		...
		FindingPath (new Vector2 (transform.position.x, transform.position.y), new Vector2 (destPos.x, destPos.y));
		switch (walkMode) {
 ...
 */ 
	IEnumerator SmoothMovePlayer ()
	{
		for (int i = 0, max = pathNodes.Count; i < max; i++) {
			bool isOver = false;
			while (!isOver) {
				Vector3 offSet = new Vector3 (pathNodes [i].pos.x + 0.5f, pathNodes [i].pos.y + 0.5f, 0) - transform.position;
				if (offSet.y > 0) {
				...
				transform.position += offSet.normalized * smoothMoveSpeed * Time.deltaTime;
				if (Vector2.Distance (pathNodes [i].pos + new Vector2 (0.5f, 0.5f), new Vector2 (transform.position.x, transform.position.y)) < 0.1f) {
					isOver = true;
					transform.position = new Vector3 (pathNodes [i].pos.x + 0.5f, pathNodes [i].pos.y + 0.5f, 0);
				}
 ...
 */ 
	...
	{
		transform.position = new Vector3 (pathNodes [pathNodes.Count - 1].pos.x + 0.5f, pathNodes [pathNodes.Count - 1].pos.y + 0.5f, 0);
	}
